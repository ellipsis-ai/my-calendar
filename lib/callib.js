...
const CalLib = {
  isTimeoutError(err) {
    return err && err.code && (err.code.includes("ETIMEDOUT") || err.code.includes("ECONNRESET"));
  },
  ...
  listCal(ellipsis, calId, rangeCallback, listCallback) {
    ...
    CalLib.retryingErrorHandler((handler) => {
      ...
      if (err) {
        throw new ellipsis.Error(`An error occurred retrieving the calendar (${err.code}): ${err.message}`, {
        ...
        cal.events.list(calId, {
          ...
          if (err) {
            throw new ellipsis.Error(`${err.code}: ${err.message}`, {
